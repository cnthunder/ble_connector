...
class BLEDeviceConnector(QMainWindow):
    ...
    def update_history_display(self):
        ...
        if device and 'name' in device and 'mac' in device:
        ...
        if os.path.exists(self.config_file):
            try:
                ...
                if 'DEFAULT' in config:
                    device_info = dict(config['DEFAULT'])
                    if 'device' in device_info and device_info['device']:
                        # 确保有必要的字段
                        result = {
                            'mac': device_info['device'],
    ...
    async def scan_devices(self):
        ...
        try:
            ADAPTER = "hci0"
            SCAN_TIME = 5  # 扫描5秒
            SERVICE_UUID = None  # 不过滤服务UUID
            VERBOSE = False
            
            # 调用 ble_serial 的扫描功能
            devices_dict = await scanner.scan(ADAPTER, SCAN_TIME, SERVICE_UUID)
    ...
    def run_connection(self, mac: str, name: str):
        ...
        try:
            ...
            args.adapter = 'hci0'
            args.mtu = 20
            args.gap_role = 'client'
            args.gap_name = ''
            args.device = mac
            args.addr_type = 'public'
            args.service_uuid = None
            args.read_uuid = None
            args.write_uuid = None
            args.mode = 'rw'
            args.write_with_response = False
            args.port = DEFAULT_PORT
            
            # 创建并运行串口循环
            from ble_serial.bluetooth.ble_client import BLE_client as BLE
            self.serial_loop_instance = self.SerialLoop(args, BLE, self.connection_signals, name)
    ...
    class SerialLoop:
        """自定义串口循环类，用于处理蓝牙连接"""
        def __init__(self, args: Namespace, BLE_class, signals: ConnectionSignals, device_name: str):
            self.args = args
        ...
        async def _run(self):
            ...
            try:
                ...
                self.signals.log_message.emit('蓝牙虚拟串口连接成功')
        ...
        async def cleanup(self):
            ...
            self.signals.log_message.emit('蓝牙虚拟串口已断开')
